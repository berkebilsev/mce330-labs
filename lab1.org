...
#+BEGIN_SRC shell
cd
mkdir mce330_spring22 # It is fine it the folder already exists.
cd mce330_spring22
mkdir demo_c_program
cd demo_c_program
touch main.c
code main.c
#+END_SRC
...
Not only that, there might be definitions that are shared across these projects that we have to manualyl update, etc.
All this process is really cumbersome.
...
- catkin_ws: the top-level directory for the catkin, all "packages" under this directory are built by catkin
  - devel: bunch of scripts
  - build: where the executables and other build artifacts go
  - src: this is where we keep our source code.
    - *CMakeLists.txt*: CMake settings for the whole packages
    - package1:
      - src
        - node1.cpp
        - node2.cpp
      - scripts
        - node3.py
        - node4.py
      - *CMakeLists.txt*: CMake settings for this package. catkin understands which files to build thanks to this file
    - package2:
    - package3:
...
"catkin_make" command is just a utility tool that initializes a catkin workspace. Once you have a catkin workspace, you don't have to initialize it ever again.
...
This file includes all the settings that catkin uses to build stuff in our workspace.
...
* Nodes
Now, let's create a simple executable.
